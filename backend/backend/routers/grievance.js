...
const { analyzeGrievanceEvidence } = require("../services/gemini");
router.post("/", checkLogin, async (req, res) => {
    ...
    const {
        ...
        category,
        subcategory,
        fileId,     // ✅ IMPORTANT (from frontend)
        mimeType    // ✅ IMPORTANT (image type)
    } = req.body;

    try {
        // ===============================
        // 🔥 STEP 1: AI SPAM CHECK (NEW)
        // ===============================
        if (fileId) {
            ...
            console.log("✅ File buffer extracted, size:", fileBuffer.length);

            // ✅ Call Gemini multimodal AI
            const aiResult = await analyzeGrievanceEvidence(
                description,
                category,
                fileBuffer,
                mimeType || "image/jpeg"
            ...
            console.log("🧠 AI Evidence Result:", aiResult);

            // ❌ BLOCK if spam
            if (aiResult.isSpam) {
                return res.status(400).json({
                    error: "❌ Uploaded evidence does not match complaint",
