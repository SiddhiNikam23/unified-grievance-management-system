...
function Sidebar({ setActivePage }) {
  ...
  return (
    <div className="min-h-screen w-auto p-5 bg-gradient-to-b from-blue-900 to-blue-600 shadow-xl backdrop-blur-md text-white">
      <ul className="space-y-3">
        ...
        <SidebarItem
          icon="📺"
          text={t("appealDashboard")}
          onClick={() => setActivePage("home")}
        />
        <SidebarItem
          icon="➕"
          text={t("lodgeGrievance")}
        ...
        />
        {}
        <li className="bg-white/20 py-5 px-4 rounded-xl">
          <div className="flex items-center space-x-2 mb-2">
            <span className="text-2xl">🌍</span>
            <span className="text-lg font-semibold">Language / भाषा</span>
          </div>
          <select
            value={language}
            onChange={(e) => changeLanguage(e.target.value)}
            className="w-full bg-white/30 text-white border border-white/40 rounded-lg px-3 py-3 cursor-pointer hover:bg-white/40 transition-all focus:outline-none focus:ring-2 focus:ring-white/50"
          >
            {languages.map((lang) => (
...
function SidebarItem({ icon, text, special, onClick }) {
  return (
    <li
      onClick={onClick}
      className={`flex justify-center items-center py-5 px-4 rounded-xl cursor-pointer transition-all duration-200 transform hover:scale-105 hover:shadow-md text-lg ${
