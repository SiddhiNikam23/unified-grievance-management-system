...
NOW ANALYZE THE ACTUAL GRIEVANCE ABOVE AND PROVIDE PRIORITY AND REASON.`;
  try {
    ...
    const reason = reasonMatch
      ? reasonMatch[1].trim()
      : "Standard priority assigned based on grievance category.";
    return {
      priority: priority,
    ...
    return fallbackPriorityDetection(
...
function fallbackPriorityDetection(description, category) {
  ...
  if (category === "law_order" || category === "health_sanitation") {
    return {
      priority: "High",
      priorityReason:
        "Category indicates potential public safety or health concern.",
    };
  }
  return {
    priority: "Medium",
    priorityReason:
      "Standard priority based on grievance category and description.",
  };
}
...
async function analyzeGrievanceEvidence(description, category, fileBuffer, mimeType) {
  try {
    ...
    const prompt = `You are an AI moderator for a government public grievance portal.
Evaluate this uploaded file evidence against the citizen's grievance description.

Grievance Description: "${description}"
Category: "${category}"

Is this image/document completely irrelevant, a stock photo, a selfie, blank, inappropriate, or obviously spam?
Note: Real grievances might just be photos of roads, broken pipes, bills, documents, or generic scenes. Be forgiving if it could potentially be related to the issue.
Only flag as spam if it is OBVIOUSLY fake or completely unrelated (like a random meme or selfie).

Return your answer purely as a JSON object:
{
  "isSpam": boolean,
  "reasoning": "brief explanation of why this was approved or rejected"
}`;
    ...
    return { isSpam: false, reasoning: "Could not parse AI response." };
  } catch (error) {
    console.error("Image Analysis Error:", error);
    return { isSpam: false, reasoning: "Evaluation failed due to technical error." };
  }
}
